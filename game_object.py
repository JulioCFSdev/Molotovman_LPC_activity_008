...
class Objects:
    class Draws:
        ...
        def collision_player(
            self, explosion_list, player_rect, duration_explosion, player_type
        ):
        ...
        def draw_wallbrk(self):
            ...
            for i in range(0, 100, 50):
                for e in range(0, 150, 50):
                    ...
                    if wallbrk in wall_coord:
                        screen.blit(
                            Bricks.images(Bricks)[1], (50 + i, 320 + e)
                        )

                    wallbrk = pygame.Rect(800 + i, 320 + e, 30, 30)
                    if wallbrk in wall_coord:
                        screen.blit(
                            Bricks.images(Bricks)[1], (800 + i, 320 + e)
                        )
            for i in range(0, 50, 50):
                for e in range(0, 150, 50):
                    ...
                    if wallbrk in wall_coord:
                        screen.blit(
                            Bricks.images(Bricks)[1], (450 + e, 650 + i)
                        )
                    wallbrk = pygame.Rect(450 + e, 50 + i, 30, 30)
                    if wallbrk in wall_coord:
                        screen.blit(
                            Bricks.images(Bricks)[1], (450 + e, 50 + i)
                        )
        ...
        def draw_bomb(pos_x_s, pos_y_s, type_soldier):
            if type_soldier == 1:
                ...
                pos_init_y = 630
            screen.blit(
                bomb.img_dynamite, (pos_x_s + pos_init_x, pos_y_s + pos_init_y)
            )
        ...
        def draw_explosion(pos_x_s, pos_y_s, type_soldier):
            ...
            screen.blit(
                bomb.explosion_up,
                (pos_x_s + pos_init_x, pos_y_s + (pos_init_y - 50)),
            )
            screen.blit(
                bomb.explosion_center,
                (pos_x_s + pos_init_x, pos_y_s + pos_init_y),
            )
            screen.blit(
                bomb.explosion_left,
                (pos_x_s + (pos_init_x - 50), pos_y_s + pos_init_y),
            )
            screen.blit(
                bomb.explosion_right,
                (pos_x_s + (pos_init_x + 50), pos_y_s + pos_init_y),
            )
            screen.blit(
                bomb.explosion_down,
        ...
        def colision_ply_brick(self, add_x, add_y, last_key_pressed):
            ...
            while x < len(brick_coord):
                if Draw_Players.draw_soviet(
                    Soviet, add_x, add_y, last_key_pressed
                ).colliderect(brick_coord[x]):
                    print("colidiu")
                x += 1
